...
try:
    ...
    pass
from helper import adjust_input, detect_first_stage_warpper, generate_bbox, nms
    ...
    def detect_face_limited(self, img, det_type=2):
        ...
        if det_type >= 2:
            ...
            for i in range(num_box):
                ...
                tmp[dy[i] : edy[i] + 1, dx[i] : edx[i] + 1, :] = img[
        ...
        points[:, 0:5] = (
            np.expand_dims(total_boxes[:, 0], 1)
            + np.expand_dims(bbw, 1) * points[:, 0:5]
        )
        points[:, 5:10] = (
            np.expand_dims(total_boxes[:, 1], 1)
            + np.expand_dims(bbh, 1) * points[:, 5:10]
        )
        ...
        for i in range(5):
            ...
            for j in range(num_box):
                ...
                tmpim[dy[j] : edy[j] + 1, dx[j] : edx[j] + 1, :] = img[
                    y[j] : ey[j] + 1, x[j] : ex[j] + 1, :
                ]
    ...
    def detect_face(self, img, det_type=0):
        ...
        if det_type == 0:
            ...
            for batch in sliced_index:
                ...
                # izip(repeat(img), self.PNets[:len(batch)], [scales[i] for i
                # in batch], repeat(self.threshold[0])) )
                local_boxes = map(
                    detect_first_stage_warpper,
                    zip(
                        repeat(img),
                        self.PNets[: len(batch)],
                        [scales[i] for i in batch],
